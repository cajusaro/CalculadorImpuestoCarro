...
public class CalculadorImpuestos {
    ...
    private RangoImpuesto buscarRangoImpuesto(double valor) {
        ...
        for (RangoImpuesto ra : rangosImpuesto){
            if (ra.contieneA(valor)) {
                rango = ra;
                break;
            }
        }
    ...
    public double calcularPago(boolean descProntoPago, boolean descServicioPublico, boolean descTrasladoCuenta) {
        ...
        double  fPorcentaje = rango.darPorcentaje();
        pago = precio * fPorcentaje / 100;
        if (descProntoPago) pago = pago - (pago * (PORC_DESC_PRONTO_PAGO / 100));
        if (descServicioPublico) pago = pago - VALOR_DESC_SERVICIO_PUBLICO;
        if (descTrasladoCuenta) pago = pago - (pago * (PORC_DESC_TRASLADO_CUENTA / 100));
    ...
    public Vehiculo buscarVehiculoMasCaro() {
        ...
        for ( Vehiculo ve : vehiculos){
            if (masCaro == null) masCaro=ve;
            if (ve.darPrecio() > masCaro.darPrecio())  masCaro = ve;
        }
        return masCaro;
    }
    ...
    public Vehiculo buscarVehiculoMasAntiguo() {
        ...
        for (Vehiculo ve : vehiculos){
            if (buscado == null) buscado = ve;
            if (Integer.parseInt(ve.darAnio()) < Integer.parseInt(buscado.darAnio())){
                buscado = ve;
                break;
            }
        }
